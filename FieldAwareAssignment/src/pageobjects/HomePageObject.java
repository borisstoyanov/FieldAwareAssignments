...
public class HomePageObject {
	...
	public void checkNavigationMenuitemsOrder(String[] expectedMenuItems) {
		...
		List<WebElement> navigationMenu = Browser.instance.findElements(By.xpath("//ul[@class='nav navbar-nav navbar-right']/li/a"));
	...
	public void checkSubMenuItemsOrder() {
		String[] menu = MenuItemsReader.getExpectedItems("ExpectedMenuItems");
		
		for (int i = 0; i < menu.length; i++){
			String[] expect = MenuItemsReader.getExpectedItems(menu[i].trim());
			String[] actual = getSubMenuItems(menu[i].trim());
			 
			for (int j = 0; j< expect.length; j++){
				Assert.assertTrue(expect[j].trim().equals(actual[j].trim()));
			}
	...
	private String[] getSubMenuItems(String menu) {
		...
		List<WebElement> subMenus = Browser.instance.findElements(By.xpath("//a[@title='" + menu + "']//../ul//div[contains(@class, 'menu_right')]//a"));
	...
	public void checkSubMenuDisplayed() {
		String[] menu = MenuItemsReader.getExpectedItems("ExpectedMenuItems");
	...
	private boolean isSubMenuDisplayed(String menuItem) {
		...
		return Browser.instance.findElement(By.xpath("//a[@title='" + menuItem + "']//../ul[@class='dropdown-menu']")).isDisplayed();
	}
